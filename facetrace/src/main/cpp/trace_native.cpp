//
// Created by zhaikn on 2019/7/19.
//
#include <jni.h>
#include <string>
...
JNIEXPORT void JNICALL
Java_com_fensite_facetrace_TraceHelper_startTracking(JNIEnv *env, jclass type)
{
...
JNIEXPORT void JNICALL
Java_com_fensite_facetrace_TraceHelper_setSurface(JNIEnv *env, jclass type, jobject surface,
                                                  jint w, jint h) {
    if (surface && w && h) {
        if (nativeWindow) {
...
JNIEXPORT void JNICALL
Java_com_fensite_facetrace_TraceHelper_destory(JNIEnv *env, jclass type) {
